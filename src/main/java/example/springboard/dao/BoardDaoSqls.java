...
class BoardDaoSqls {
    static final String GET_BOARD_LIST_ALL = "SELECT id, origin_id, depth, reply_seq, category_id, member_id, title, ip_addr, reg_date " +
            "FROM board WHERE category_id = :category_id ORDER BY origin_id DESC, reply_seq ASC";
    static final String GET_BOARD_LIST_BY_MEMBER = "SELECT b.id, m.name, b.title, b.reg_date, b.ip_addr " +
            "FROM board AS b, member AS m WHERE category = :category and b.member_id = :m.id " +
            "ORDER BY b.origin_id DESC, b.reply_seq ASC";
    static final String GET_BOARD_LIST_BY_TITLE = "SELECT b.id, m.name, b.title, b.reg_date, b.ip_addr " +
            "FROM board AS b INNER JOIN member AS m ON b.member_id = m.id " +
            "WHERE b.category_id = :b.category_id AND b.title LIKE '%:b.title%' " +
            "ORDER BY b.origin_id DESC, b.reply_seq ASC";
    static final String GET_BOARD_LIST_BY_CONTENT = "SELECT b.id, b.member_id, b.title, b.reg_date, b.ip_addr " +
            ...
            "ORDER BY b.origin_id DESC, b.reply_seq ASC";
    static final String GET_BOARD_LIST_BY_TITLE_OR_CONTENT = "SELECT b.id, b.member_id, b.title, b.reg_date, b.ip_addr " +
            "FROM board AS b INNER JOIN board_body AS bb ON b.id = bb.id " +
